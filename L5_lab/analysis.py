...
class DataReader:
    ...
    @staticmethod
    def read_data(sim_folder, sim_file_name, save_every_ms, h_dt, h_tstop):
        
        # For convenience
        if sim_file_name.endswith(".h5"):
            sim_file_name = sim_file_name[:-3]

        step_size = int(save_every_ms / h_dt) 
        steps = range(step_size, int(h_tstop / h_dt) + 1, step_size)
        ...
        for step in steps:
            with h5py.File(os.path.join(sim_folder, f"saved_at_step_{step}", sim_file_name + ".h5"), 'r') as file:
                ...
                # Traces
                    ...
                    data.append(retrieved_data[:, :save_every_ms])
